...
export default function MedicamentosView() {
  const datos = [
    { id: 'MED-01', nombre: 'Amoxicilina 250mg', tipo: 'Antibiótico', stock: '45 unidades', precio: '$12.000' },
    { id: 'MED-02', nombre: 'Meloxicam Gotas', tipo: 'Analgésico / Antiinflamatorio', stock: '12 unidades', precio: '$18.500' }
  ];
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      <div className="flex items-center justify-between w-full">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold text-gray-800 tracking-tight">Medicamentos</h1>
          </div>
          <span className="text-sm text-gray-500 font-medium">{datos.length} fármacos en farmacia</span>
        </div>
      ...
      <div className="w-full max-w-xl relative">
        ...
        <input type="text" placeholder="Buscar medicamentos..." className="w-full pl-12 pr-4 py-3 bg-[#fafaf6] border border-gray-200 rounded-2xl text-sm focus:outline-none focus:border-emerald-700 font-medium text-gray-700 transition-colors" />
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-200/60 text-xs font-bold text-gray-400 tracking-wider bg-gray-50/50">
                ...
                <th className="py-4 px-6">PRECIO AL PÚBLICO</th>
            ...
            <tbody className="text-sm font-medium text-gray-700 divide-y divide-gray-100">
              {datos.map((reg) => (
                <tr key={reg.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-4 px-6 text-gray-500">{reg.tipo}</td>
                  <td className="py-4 px-6 text-amber-800 font-semibold">{reg.stock}</td>
                  <td className="py-4 px-6 font-bold text-gray-800">{reg.precio}</td>
